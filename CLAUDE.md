...
- **Two-way book sync** (opt-in via the `webdav_autosync_books_two_way` toggle; only affects book sync, not progress sync — progress sync is always bidirectional):
  ...
  - Change detection: remote-changed if etag or mtime differs from cache; local-changed if size differs OR mtime drifts by **≥ 2 seconds** from cache. Both changed = conflict. The mtime tolerance was added in v1.5.1 — without it, filesystems with coarse timestamp granularity (vfat/exFAT, used on the user-storage partition of older Kindles) would round the post-write mtime on flush, so the next stat returned a value different from what `stat_local` cached at write time, marking every unmodified sidecar as "locally changed" and triggering a full re-upload on every progress sync. Symptom on those devices: opening + closing one book caused all sidecars across the library to re-upload at the next manual sync. Keep the tolerance — don't reintroduce a strict mtime equality check. Size is the decisive signal; mtime only needs to be "near enough." `diff_indices` also `logger.dbg`s the cached vs. current values whenever a file is flagged as locally-changed, so the same class of bug stays diagnosable from `crash.log` if it ever recurs on a different filesystem.
