...
- **UI feedback shape per trigger** — runners take `show_syncing_message` (default true) and `chain_stats` (optional accumulator). What each trigger sets:
  - **Manual** (menu / Dispatcher): both flags default — runner shows its own "Syncing…" / "Syncing reading progress…" InfoMessage AND its own per-runner summary popup. Same UI as before this work.
  - **Startup, both syncs on**: orchestrator shows ONE combined "Syncing…" InfoMessage at chain start; both runners suppress their own (`show_syncing_message=false`) and accumulate counts in `chain_stats`; orchestrator closes the InfoMessage and shows ONE merged summary via `showChainSummary` at chain end.
  - **Startup, one sync on**: standalone — runner's own UI as in the manual case.
  - **Resume**: every branch (chain, progress only, book only) passes `show_syncing_message=false`. The chain branch additionally uses `chain_stats` for a single merged summary; standalone branches keep their per-runner summary. Net effect on wake: no "Syncing…" pop-in, summary still surfaces.
  - **Close**: `doProgressSyncForBook`, separate code path, fully silent (no InfoMessage, no summary, no chain).
  - `plan_progress` and `plan` (book sync) failures: standalone runs show their own InfoMessage; chain runs fold the failure into `chain_stats.failures` as one synthetic entry so the next runner still gets to run and the merged summary surfaces both. Don't reintroduce a chain-mode failure popup — it'd defeat the single-popup contract.
  - `mergeChainStats(chain_stats, stats)` is the canonical accumulator merge — all runners route through it. One-way book sync's `skipped` field maps to `unchanged`; one-way `err` (single string) folds as one synthetic `failures` entry. Don't add per-runner summary calls inside chain branches — `chain_stats ~= nil` is the silencing predicate.
