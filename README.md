...
- **Wake intentionally hides the "Syncing…" indicator.** The plugin still runs the sync and shows the summary popup at the end, but the *during* indicator is suppressed on wake so the device doesn't barge in with a popup the moment you pick it up. Startup keeps the indicator. Manual keeps the indicator. Close is fully silent (no indicator, no summary). When both reading-progress and book sync run together (on wake or startup), they share **one** indicator (startup only — wake has no indicator) and produce **one** combined summary at the end instead of two of each.
